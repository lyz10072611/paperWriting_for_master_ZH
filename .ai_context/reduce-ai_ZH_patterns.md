...
name: reduce-ai_ZH
description: |
  去除文本中的 AI 生成痕迹，使文字更自然、更像人类书写。基于维基百科"AI 写作特征"指南，覆盖 24 种模式。
  含中文学术论文写作补充（禁用套话、学术克制、人类逻辑）。用于编辑审阅文本、润色论文、去 AI 味时自动应用。
allowed-tools:
  - Read
  - Write
  - Edit
  - AskUserQuestion
metadata:
  trigger: 编辑或审阅文本，去除 AI 写作痕迹；撰写或润色中文学术论文时自动应用
  source: 翻译自 blader/humanizer，参考 hardikpandya/stop-slop，融合 paper-writing 学术论文补充
---

# reduce-ai_ZH: 去除 AI 写作痕迹

你是一位文字编辑，专门识别和去除 AI 生成文本的痕迹，使文字听起来更自然、更有人味。本指南基于维基百科的"AI 写作特征"
...
- **不要把“去 AI 味”等同于“压缩内容”**。方法、数据和实验部分首先要保证解释完整、逻辑清楚、可复现。
- **优先删除空泛评价，不优先删除技术说明**。像「具有重要意义」「广阔应用前景」应优先清理；像步骤目的、参数来源、筛选依据、因果关系不应被顺手删掉。
- **允许保留有功能的连接句**。若「考虑到…因此…」「为避免…」「用于…」承担真实的逻辑解释功能，应保留或自然改写，不要机械视为 AI 味。
- **方法段润色标准**：去掉套话后，读者仍应能回答三个问题:
  1. 为什么这样处理
  2. 每一步做了什么
  3. 这样处理对后续有什么作用
- **审稿人优先**：如果“更像人写的版本”削弱了方法说明，那就是失败的润色。
...
- ✓ **使用了"此外""然而"等连接词？** 考虑删除
- ✓ **三段式列举？** 改为两项或四项
- ✓ **方法段是否因为润色而丢失“步骤-作用-目的”链条？** 如果是，补回
- ✓ **数据段是否因为压缩而弱化了来源、尺度、条件或数量？** 如果是，补回
...
当处理**硕士/博士毕业论文**等学术写作时，除上述 24 种模式外，还须遵守以下中文学术特有规则：

### 学术场景下「个性与灵魂」的调整

- **人称**：学术论文统一使用「本文」「本研究」「本设计」作主语，禁用「我」「我们」
- **第一人称**：上文「适当使用我」不适用于中文学位论文正文，仅适用于博客、随笔等非正式文体
...
- 用**具体事实或数据**替代抽象评价：「该方法的 mAP 达到 92.3%，优于基线 5.1 个百分点」而非「该方法效果显著」
- 用**因果链条**替代空泛过渡：先给现象/数据，再给解释，最后给结论
- 用**短句+长句交错**替代单一节奏
- **一段一事**：每段聚焦一个论点，不塞入多个并列主题
- **主谓宾清晰**：少用「的」字长串（如「基于…的…的…的方法」），必要时拆句
- **术语一致**：全文同一概念用同一术语
...
**AI 味：** 在当今遥感技术快速发展的背景下，目标检测成为了研究的热点，具有广阔的应用前景。

**人类风格：** 遥感影像分辨率近年已普遍达到亚米级，但大范围目标自动识别仍依赖人工判读，效率难以满足业务需求。因此，发展高效、可靠的目标检测方法成为实际工作中的迫切需求。
...
中文学术论文补充规则融合自 paper-writing 等学术写作规范。
